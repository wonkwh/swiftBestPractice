# Swift Style Guide.
 - https://github.com/uxight/swift-style-guide fork한 문서를 수정적용한 문서... 
...
Explicitly extending lifetime is Good to optional chaining.
...
여러개의 옵셔널을 `guard` 나 `if let` 을 사용해 언랩(unwrapped)할 때 구문의 중첩 사용을 최소화하고 변수 사이에 `,` 를 사용해 하나의 구문안에서 해결한다. 이 때, `guard` 다음 변수(혹은 조건)을 선언할 땐 새로운 줄에 들여쓰기를 한 후 시작하고 `else` 는 다음 줄에 새로 적는다. 
